...
function initPanelProblemas() {
    ...
    toggleProblemas.addEventListener('click', () => {
        ...
        if (!problemasContent.classList.contains('hidden') && problemasContent.innerHTML === '') {
            renderProblemas();
        }
    });
}
...
function createHiloBlock(hilo, index) {
    ...
    dragHandle.className = 'thread-handle';
    ...
    factsList.className = 'px-4 space-y-2';
    
    hilo.hechos.forEach(hecho => {
